...
document.addEventListener('DOMContentLoaded', () => {
  // Device table logic (continued from Part 1)
  // ...
  ...
  if (installerCanvas && calfireCanvas) {
    ...
    document.querySelector('form').addEventListener('submit', function(event) {
      if (installerSignaturePad.isEmpty() || calfireSignaturePad.isEmpty()) {
        event.preventDefault();
        alert('Please provide signatures.');
      } else {
        document.querySelector('input[name="installer_signature"]').value = installerSignaturePad.toDataURL();
        document.querySelector('input[name="calfire_signature"]').value = calfireSignaturePad.toDataURL();
      }
    });

    document.getElementById('generatePdf').addEventListener('click', function() {
      const formData = new FormData(document.querySelector('form'));
      fetch('includes/pdf_generation.php', {
        method: 'POST',
        body: formData
      })
